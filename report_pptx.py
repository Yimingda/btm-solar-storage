...
• Huawei Red (#CF0A2C) as the primary accent — KPI numbers, chart bars,
  left-border strips, section markers
...
_HRD  = "CF0A2C"   # Huawei red   — KPI values, chart bars, accent strips
...
def _kpi_block(slide, x, y, label: str, value: str,
               sub: str = "", w: float = 2.8):
    """KPI card: Huawei-red value, thin red left strip."""
...
def _s1_cover(prs, project_name: str, client_name: str,
    ...
    _tb(slide, 9.22, 1.70, 3.90, 0.85,
        f"{pv_kwp:,.0f} kWp", 40, bold=True, color=_HRD, align="center")
...
def _s4_financial(prs, results: dict, fin_df, company: str):
    ...
    _header_bar(slide, headline,
                ...
                "Section 12B 150% Year-1 allowance applied")
    ...
    _narrative(slide, 0.28, 5.82, 12.78, 0.62,
               ...
               "BESS SOH decline.  Section 12B 150% deduction in Year 1 materially "
               "improves early cash flows.  NPV remains positive across ±2% "
               "tariff-escalation sensitivity (available in the full Excel model).")
...
def generate_pptx(
    ...
    """
    Generate an 8-slide executive PPTX report (Huawei Digital Power style).
